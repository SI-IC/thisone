...
### Фаза 6 — CC-плагин: manifest, SessionStart-хук, wire/unwire, skill, commands (done, 2026-08-05)

Построено: `claude-plugin/lib/vite-config-patch.mjs` (`addPlugin`/`removePlugin` — идемпотентные string-патчи `vite.config.*`; ключевая деталь: перед поиском ключа `plugins:` источник **маскируется** — `maskCommentsAndStrings` заменяет содержимое комментариев/строк пробелами той же длины [индексы не съезжают], поиск `plugins:`/парных скобок идёт по маске, а вставка — по оригинальному тексту теми же индексами; поиск дополнительно ограничен телом `defineConfig(`/`export default {` объекта. CJS-конфиги [`module.exports`] детектятся и патч отклоняется с явной инструкцией вместо порчи файла ESM-import'ом), `claude-plugin/lib/project.mjs` (общие `PKG_NAME`/`CONFIG_NAMES`/`detectPackageManager` — вынесены из wire/unwire, чтобы не дублировать), `claude-plugin/scripts/wire.mjs` (`inspectProject(dir)` чистая detect-функция + `wire(dir)`: install через pnpm/npm/yarn по lockfile github-тега [резолв через `git ls-remote --tags --refs`, таймаут 60с] + патч конфига), `claude-plugin/scripts/unwire.mjs` (обратная операция), `claude-plugin/hooks/session-start.sh` (`node wire.mjs || true`), `claude-plugin/skills/claude-feedback/SKILL.md`, `claude-plugin/commands/{feedback,feedback-setup,feedback-remove}.md`, `claude-plugin/.claude-plugin/plugin.json` (добавлены `mcpServers`/`hooks.SessionStart`/`skills`/`commands`).

**Packaging-риск Ф5 закрыт:** `scripts/build.mjs` получил третий шаг — esbuild бандлит `claude-plugin/mcp-server.mjs` (+ `lib/bridge-client.mjs`, + `@modelcontextprotocol/sdk`) в self-contained `claude-plugin/mcp-server.bundled.mjs` (createRequire-banner как в Ф2/Ф4-граблях), build.mjs проверяет что в выводе не осталось голого `import ... from "@modelcontextprotocol/sdk"`. `plugin.json` `mcpServers.claude-feedback.args` указывает на `mcp-server.bundled.mjs`, а не на исходник — marketplace-install без `node_modules` теперь работает (проверено: скопировал бандл в директорию без node_modules, `node --check` + реальный запуск процесса — стартует). **Важно:** бандл лежит в `claude-plugin/` (НЕ в поддиректории `dist/`), потому что `mcp-server.mjs` читает версию через `__dirname`-relative путь до `.claude-plugin/plugin.json` — перенос в поддиректорию сломал бы этот путь.

**Промахи манифеста (2):** `vite-config-patch.mjs` положен в `claude-plugin/lib/` вместо `claude-plugin/scripts/lib/` из манифеста — так он рядом с уже существующим `claude-plugin/lib/bridge-client.mjs` (Ф5), а не в новой вложенной директории; `claude-plugin/lib/project.mjs` создан вне манифеста (появился на self-review-шаге для дедупликации wire.mjs/unwire.mjs).

**Self-review (1 проход, 3 сабагента — code+security+premortem):** code-review и premortem **независимо** нашли одну и ту же CRITICAL-находку — `findPluginsArrayRange` матчил первое текстовое вхождение `plugins:` по всему файлу, включая комментарии («// note: plugins: …» перед реальным массивом) и несвязанные ключи (`resolve.alias` перед настоящим `plugins:`), тихо патчил не тот массив, при этом `configWired`-флаг всё равно становился `true` — **applied**: переписан на masked-поиск (см. выше), ограниченный телом конфиг-объекта; тест `does not hijack an unrelated array before a comment` добавлен. Premortem также поймал CJS-конфиг (`module.exports`) — ESM `import` вставлялся бы в CommonJS-файл и ломал `vite dev` синтаксической ошибкой — **applied**: `isCommonJs`-детект + graceful bail с note; тест добавлен. code-review нашёл дублирование `detectPackageManager`/`CONFIG_NAMES`/`PKG_NAME` между wire.mjs/unwire.mjs — **applied**: вынесено в `project.mjs`. premortem указал на `execFileSync` без таймаута в install/uninstall/`git ls-remote`, вызываемые из SessionStart-хука у каждого пользователя — **applied**: `timeout: 60_000` на все три вызова. security-review не нашёл command-injection/path-traversal (execFileSync с argv-массивами, без shell; regex тега якорится на `v\d+\.\d+\.\d+`; CONFIG_NAMES — фиксированный whitelist) — **rejected**: тег-mutability/no-consent-install риск отмечен как design-level и уже принят в преамбуле плана («Риски» → «Авто-`pnpm add github:` в SessionStart-хуке трогает чужой проект… Смягчение: Ф6 — строгая идемпотентность») — не переоткрываю. code-review нашёл `compareTags` не переживёт pre-release теги (`v1.2.0-beta.1`) — **rejected**: теги создаёт исключительно `release.mjs` этого репо, всегда строгий `vMAJOR.MINOR.PATCH`, сторонних тегов в этом упражнении не бывает. premortem указал, что при нескольких найденных config-файлах берётся первый по фиксированному порядку, не совпадающему с реальным разрешением Vite — **rejected**: план явно предписывает именно это поведение («несколько конфигов → первый найденный, лог»).

**Verify-гейт (исполнено реально, всё зелёное):** `tests/smoke/wire-smoke.mjs` — временный project-dir (package.json vue+vite + vite.config.ts) → `wire.mjs` с `CLAUDE_FEEDBACK_SKIP_INSTALL=1` патчит конфиг (`claudeFeedback` + import-строка присутствуют) → `unwire.mjs` возвращает исходный вид (без `claudeFeedback`, `vue()` на месте) → манифест `plugin.json` валиден и содержит `mcpServers`+`hooks` → `mcp-server.bundled.mjs` не содержит голого `@modelcontextprotocol/sdk`-импорта + `node --check` проходит → доп. project-dir без vite (только `{}` в package.json) не роняет `wire.mjs` → `wire-smoke ok`. Плюс `pnpm test:run` → **124 passed** (13 файлов: +vite-config-patch 11 + wire-detect 6 поверх Ф1–Ф5 107); `node tests/smoke/{bridge,mcp,wire}-smoke.mjs` → все ok; `node tests/smoke/overlay-smoke.mjs` → `overlay-smoke ok` (тот же пост-print зависший процесс, что и раньше — известная особенность esbuild-сервиса из Ф4, не регрессия); `tsc -p tsconfig.json --noEmit` → exit 0; `pnpm build` → dist + `mcp-server.bundled.mjs`. **Откатов не было.**
